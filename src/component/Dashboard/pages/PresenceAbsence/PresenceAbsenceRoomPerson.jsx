...
class PresenceAbsenceRoomPerson extends Component {
    ...
    componentDidMount = async () => {
        axios.get(`http://localhost:8089/api/v1/supervisor/unit/person/${this.context.unitId}`, {headers: {'Authorization': localStorage.getItem('accessToken')}}).then(response => response.data)
    ...
    handleGetStatus = async (checked, room, info) => {
        ...
        axios.post('http://localhost:8089/api/v1/supervisor/record', report, {headers: {'Authorization': localStorage.getItem('accessToken')}}).then(response => response.data)
            .then((data) => this.setState({
                loading: false
            })).catch(() => {
            if (localStorage.getItem('role') === 'MANAGER') {
