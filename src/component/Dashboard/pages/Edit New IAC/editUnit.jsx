...
class EditUnit extends Component {
    ...
    render() {
        return (
            <>
                    ...
                    <div className="row pb-5">
                        {this.state.floor.map((f, i) => (
                            <div className="col-md-4 col-sm-6 col-xs-12 p-3">
                                <div className="floor-box">
                                    <button className="floor-close-btn" onClick={() => {
                                        this.handleDeleteShowFloor(f)
                                    }}><AiFillCloseCircle color="#F1416C"/></button>
                                    <div className="row">
                                        <div className="col-7 p-0">
                                            <div className="title">
                                                <EditText ref="floorTitle" showEditButton defaultValue={f.name}
                                                          onSave={this.editFloorTitle}/>
                                            </div>
                                        </div>
                                        <div className="col-5">
                                            <div className="addAccessory">
                                                <button className="addAccessoryBtn" onClick={() => {
                                                    this.hanldeFloorAccShow(f)
                                                }}>امکانات طبقه <MdAddCircle fontSize="15px"/></button>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div className="col-4">
                                                <div className="unit-box">
                                                    <button className="unit-close-btn" onClick={() => {
                                                        this.handleDeleteShowUnit(u, i)
                                                    }}><AiFillCloseCircle color="#F1416C"/></button>
                                                    <TbBuilding fontSize="2rem"/>
                                                    <div className="title"><EditText className="editable" showEditButton
                                                                                     onSave={this.editUnitTitle}
                                                                                     defaultValue={u.number}/></div>
                                                </div>
                                            </div>
                                        ))}
                                        <button onClick={() => {
                                            this.addUnit(f)
                                        }} className="col-4 unit-add-btn">
                                            <div className={"d-flex align-items-center"}
                                                 style={{color: "#296d9a"}}>افزودن واحد<AiOutlinePlus size={25}
                                                                                                      className={"mx-2 mb-1"}/>
                                            </div>
                                        </button>
                                    </div>
                        ...
                        <div className="col-4 p-3">
                            <div className="floor-box add">
                                <button onClick={this.addFloor} className="btn">
                                    <div className={"d-flex align-items-center"}
                                         style={{color: "#296d9a", fontSize: "1.5rem"}}>افزودن واحد<AiOutlinePlus
                                        size={25} className={"mx-2 mb-1"}/></div>
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <Modal centered show={this.state.showDeleteModalFloor}
                       onHide={() => this.setState({showDeleteModalFloor: false})}>
                    ...
                    <Modal.Footer className="justify-content-start">
                        ...
                        <button className="btn btn-light"
                                onClick={() => this.setState({showDeleteModalFloor: false})}>بستن
                        </button>
                    </Modal.Footer>
                </Modal>
                ...
                <Modal centered show={this.state.showَFloorAccessory} onHide={() => {
                    this.handleFloorAccClose()
                }}>
                    <Modal.Header closeButton>
                            ...
                            <div className="accessory row">
                                <div>
                                    <button className="close-btn" onClick={() => {
                                        this.deleteAccessory(accessory, this.state.floorTemp)
                                    }}><AiFillCloseCircle color="#F1416C"/></button>
                                </div>
                                ...
                                <div className="accessory-count col-5">
                                    <CounterInput min={0} max={10} count={accessory.count} onCountChange={count => {
                                        this.handleCount(count, accessory, this.state.floorTemp)
                                    }}/>
                                </div>
                            </div>
                        ))}
                    ...
                    <Modal.Footer className="justify-content-start">
                        <button className="btn btn-success" onClick={() => {
                            this.handleSubmitAcc(this.state.floorTemp)
                        }}>ثبت
                        </button>
    ...
    addFloor = async () => {
        ...
        const rawResponse = await axios.post('https://api.saadatportal.com/api/v1/supervisor/floor', {
            name: `${count}`,
            empty: "true"
                ...
                if (localStorage.getItem('role') === 'MANAGER') {
                    axios.get('https://api.saadatportal.com/api/v1/manager/token/refresh', {headers: {'Authorization': localStorage.getItem('refreshToken')}})
                        .then((response) => {
                            if (response.headers["accesstoken"]) {
                                localStorage.setItem("accessToken", response.headers["accesstoken"]);
                                axios.post('https://api.saadatportal.com/api/v1/supervisor/floor', {
                                    name: `${count}`,
                                    empty: "true"
                                }, {headers: {'Authorization': localStorage.getItem('accessToken')}}).then(response => response.data)
                                    .then((data) => {
    ...
    addUnit = async (f) => {
        ...
        const rawResponse = await axios.post('https://api.saadatportal.com/api/v1/supervisor/unit', {
            number: count,
            empty: "true",
            floorId: f.id
        }, {headers: {'Authorization': localStorage.getItem('accessToken')}}).then(response => response.data)
    ...
    editFloorTitle = async ({value, previousValue}) => {
        ...
        await axios.put(`https://api.saadatportal.com/api/v1/supervisor/floor/${floor.id}`, {
            name: value,
            empty: "true"
        }, {headers: {'Authorization': localStorage.getItem('accessToken')}}).then(response => response.data)
            .then((data) => {
                ...
                if (localStorage.getItem('role') === 'MANAGER') {
                    axios.get('https://api.saadatportal.com/api/v1/manager/token/refresh', {headers: {'Authorization': localStorage.getItem('refreshToken')}})
                        .then((response) => {
                            if (response.headers["accesstoken"]) {
                                localStorage.setItem("accessToken", response.headers["accesstoken"]);
                                axios.put(`https://api.saadatportal.com/api/v1/supervisor/floor/${floor.id}`, {
                                    name: value,
                                    empty: "true"
                                }, {headers: {'Authorization': localStorage.getItem('accessToken')}}).then(response => response.data)
                                    .then((data) => {
