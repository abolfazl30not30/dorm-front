...
class contacts extends Component {
    ...
    async componentDidMount() {
        axios.get('http://localhost:8089/api/v1/supervisor/phoneBook', {headers: {'Authorization': localStorage.getItem('accessToken')}}).then(response => response.data)
    ...
    handleSearchInput = async (e) => {
        ...
        axios.get(`http://localhost:8089/api/v1/supervisor/phoneBook/search?${this.state.searchType}=${e.target.value}`, {headers: {'Authorization': localStorage.getItem('accessToken')}}).then(response => response.data)
            .then((data) => this.setState({
                contacts: data,
                searchLoading: false
            })).catch(() => {
    ...
    handleRecordContact = async () => {
        ...
        await axios.post('http://localhost:8089/api/v1/supervisor/phoneBook', newContact, {headers: {'Authorization': localStorage.getItem('accessToken')}}).then(response => response.data)
            .then((data) => this.setState({
                loading: false
            })).catch(() => {
                if (localStorage.getItem('role') === 'MANAGER') {
